...
- (void)viewDidLoad {
    ...
    self.searchBar.tintColor = HJWColor(32, 191, 179);
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    MTCityGroup *group = self.cityGroups[indexPath.section];
    // 发出通知
    [NOTI_CENTER postNotificationName: @"MTCityDidChangeNotification"
                               object:nil
                             userInfo:@{@"MTSelectCityName":group.cities[indexPath.row]}];
    [self dismissViewControllerAnimated:YES completion:nil];
}
@end
